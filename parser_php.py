...
def p_comment(p):
    '''comment : OPENCOMMENT commentRep CLOSECOMMENT'''
    pass

def p_commentRep(p):
    '''commentRep : FUNCTIONEXP
                    | FUNCTIONEXP commentRep'''
    pass 
...
def p_error(p):
    if VERBOSE:
        if p is not None:
            print (chr(27)+"[1;31m"+"\t ERROR SINTACTICO - NO SE ESPERABA EL TOKEN" + chr(27)+"[0m")
            print ("\t\tLine: "+str(p.lexer.lineno)+"\t=> "+str(p.value))
        else:
            print (chr(27)+"[1;31m"+"\t ERROR: ERROR SINTACTICO"+chr(27)+"[0m")
